...
const STRATEGY_CATALOG: readonly ApiRuntimeExecutionStrategyCatalogEntry[] = [
  ...
  },
  {
    ...
    runtimeOwnedExecution: true,
    runtimeHostedBackends: [...RUNTIME_HOSTED_BACKENDS],
    description: 'Runtime-owned describe/explain/plan/select loop with bounded constraint-aware replanning.',
    requestSupport: {
      acceptanceCriteria: true,
      strategyContext: true,
      correlation: true,
    ...
    contextSchema: [
      buildIntegerContextField(
        'maxSteps',
        'Caps the number of describe/explain/plan/select cycles before the runtime returns a step-limit failure.',
        {
          defaultValue: DEFAULT_MAX_TOOL_STEPS,
          defaultSources: ['instance.maxToolSteps', 'runtime.defaultMaxToolSteps'],
        },
      ...
      buildIntegerContextField(
        'timeoutMs',
        'Bounds the full runtime-owned DEPS loop when the provider instance exposes a timeout budget.',
        {
          defaultSources: ['instance.timeoutMs'],
        },
      ),
      buildIntegerContextField(
        'stuckThreshold',
        'Trips repeated-selection stuck detection after this many duplicate tool-call batches.',
        {
          defaultValue: DEFAULT_STUCK_THRESHOLD,
          defaultSources: ['runtime.defaultStuckThreshold'],
        },
    ...
    notes: [
      'The runtime keeps DEPS product-neutral by only owning the execution loop, not any product task graph.',
    ],
  },
] as const;
