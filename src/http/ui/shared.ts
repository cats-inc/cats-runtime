...
(function() {
  ...
  function normalizePlaygroundAgentSelection(input) {
    ...
    if (!entryId && (provider === 'cursor' || provider === 'copilot' || provider === 'opencode' || provider === 'kilo' || provider === 'devin' || provider === 'cline' || provider === 'kiro' || provider === 'junie' || provider === 'auggie') && options.allowLegacyModel === true
