...
import { supportsQuotaRefresh } from '../../core/usage/usageSnapshot.js';
...
usageRoutes.post('/usage/refresh', bodyLimit({ maxSize: 1024 }), async (c) => {
  ...
  if (Object.keys(input).some((key) => key !== 'provider' && key !== 'instance')
    || !supportsQuotaRefresh(input.provider) || typeof input.instance !== 'string' || !input.instance || input.instance.length > 100
    || !Object.hasOwn(ctx.config.providerInstances?.[input.provider] ?? {}, input.instance)) {
    return c.json({ error: 'invalid_quota_target' }, 400);
  }
  if (!ctx.quotaRefresh) return c.json({ error: 'quota_refresh_unavailable' }, 503);
  const result = await ctx.quotaRefresh.refresh({ provider: input.provider, instance: input.instance, backend: 'cli' });
