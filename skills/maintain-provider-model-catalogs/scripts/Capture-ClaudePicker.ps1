...
.DESCRIPTION
    ...
    starting level. Only arrow keys are sent: Enter saves a default and "s" applies the
...
.PARAMETER ExpectedModelCount
    Number of rows independently observed for this account. It does not prove completeness.
...
# separators use \s rather than a literal space.
...
try {
    ...
    if ($initial.Rows.Count -ne $ExpectedModelCount) { throw 'Visible model count differs from expected coverage.' }
    for ($i=0; $i -lt $initial.Rows.Count; $i++) {
        if ([int]$initial.Rows[$i].Groups['index'].Value -ne $i + 1) { throw 'Visible model rows are incomplete or out of order.' }
    }
    foreach ($row in $initial.Rows) {
        $index = [int]$row.Groups['index'].Value
        $columns = $row.Groups['rest'].Value.Trim() -split '[ \t]{2,}', 2
