...
## Usage and Account-Quota Track (U1 / Passive Signals Implemented)

- [x] Record the usage/quota distinction and runtime ownership in ADR-038.
- [x] U1: expose a stable, sanitized snapshot over the existing bounded execution
...
Usage UI belongs to `cats-apps`; App permissions and the host bridge belong
...
Existing usage/incident/guardrail support remains shipped. Passive Claude/Codex
remaining-percent/reset observations are now exposed; active full-account quota
collection and durable historical coverage are not implied.
...
- `scripts/` and `.github/workflows/` (maintainer-side watch job plus the
  preflight heartbeat check, PLAN-036)
