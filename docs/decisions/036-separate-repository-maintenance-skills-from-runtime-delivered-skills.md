...
- `.gemini/skills/` is **not** read, so `GEMINI.md`'s claim was wrong at decision
  time and ADR-107's default action of dropping that target is still correct;
  Antigravity later corrected its owned file in `193a37b`
...
During implementation, the owner also directed the work to the working cross-agent skills in the
sibling `quant-x` repository. Its accepted SPEC-001/PLAN-001 outcome and commit `c4e523a` record an
independent Grok probe: Grok, like Codex and Antigravity, discovers project skills from
`.agents/skills/`. This adds another consumer of the same mirror, not another discovery path.
...
3. **Agent-specific paths remain generated discovery mirrors.**
   - `.agents/skills/` is read by Codex, Antigravity (`agy`), and Grok; it is a
   ...
   - there is no third mirror. `.gemini/skills/` is not read by agy (see
     ...
     corrected rather than migrated. Antigravity and Grok need no targets of
     their own because they already read `.agents/skills/`. The
     `--agent antigravity` and `--agent grok` names are aliases for that same
     directory; they deliver no additional copy.
...
## Implementation Amendment: Keep Generated Workspace Helpers Generic

During implementation on 2026-09-01, PLAN-037 found that
`WorkspaceSubstrateService` also generates skill-sync helpers into arbitrary user workspaces. Those
templates are product surface, while `developer-skills/` is a cats-runtime repository convention.

The implementation therefore uses PLAN-037's split option:

- the three tracked cats-runtime entrypoints delegate to one repository-specific reconciler and
  sync `developer-skills/`;
- helpers generated by `WorkspaceSubstrateService` continue to sync a generic workspace's
  `skills/` root;
- source comments and tests pin that difference so later maintenance does not accidentally make one
  contract imitate the other.

This narrows decision point 4 to cats-runtime's tracked repository helpers. It does not remove the
...
*Amended: 2026-09-01 during the first implementation — cats-runtime's tracked helpers use
`developer-skills/`, while generated workspace helpers retain their generic `skills/` contract;
the owner-provided working quant-x reference confirms Grok as another `.agents/skills/` consumer.*
