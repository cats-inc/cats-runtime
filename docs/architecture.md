...
2. **Machine detection** — runtime-owned observations, scoped to selected
   targets. Completed results retain their observation times; they are not
   live availability guarantees.
3. **Enabled config** — operator intent about which targets Cats may use,
   independently of detection. Persisted as `providers.yaml` (ADR-039).
...
The observation archive retains one last completed result per exact target and
a private configuration digest. Setup reads project it as unchanged, changed,
or not selected without probing. Selection edits preserve matching evidence;
changed commands/endpoints require detection, and unselected history cannot
authorize work. Scan snapshots and late-result admission remain revision-scoped.
...
- Exposes policy-neutral workspace substrate operations (`audit-workspace`,
  ...
  primitives together with matching local-tool/HTTP/MCP discovery surfaces,
  including the profile catalog and `/workspace/substrate/*` execution routes
...
- Reuses that same shared runtime session inspection to project bounded
  ...
  `outputDir`, bounded semantic run summaries when `resultSummary` exists, and
  bounded open browser-page samples instead of only browser session counts
...
13. Startup/readiness state is exposed over `GET /health`, while
   ...
   truth, per-tool capability/access catalog truth for runtime-local tool
   surfaces, and bounded tooling observability truth,
   ...
   `tools.effective` follow-through on `GET /providers/{provider}/tools` and
   session-aware live `GET /diagnostics/providers` reads for agent targets that
   support it, and additive
