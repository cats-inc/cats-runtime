...
- [x] Aider — install tier landed; execution refused on settled evidence. 0.86.2 has no machine-readable output, no ACP/MCP/server mode, exits 0 even when the model call fails, and runs `git init` in the working directory. Auth is multi-source including an undocumented OpenRouter OAuth store at `~/.aider/oauth-keys.env`. Probe: `docs/research/2026-08-09-aider-cli-probe.md`.
...
| 2026-08-09 | Drove the Cline adapter's assembled invocation through `buildProcessSpawnConfig` into the real 3.0.51 binary. The argv is accepted and the spawn config resolves, closing that half of the outstanding end-to-end check. It also found a defect the merged-stream fixtures had hidden: Cline writes `run_*` lines to stdout but `error` lines to stderr, and `WorkerProcess` only parses stdout, so the adapter's decision to let a non-completed `run_result` yield to the trailing error line produced no terminal event at all — the turn degraded to a turn-timeout instead of reporting the cause. A non-completed `run_result` now terminates the turn from its own text; re-verified live. A successful authenticated turn remains blocked on account credit. |
| 2026-08-09 | Aider 0.86.2 install tier landed with execution refused on settled evidence: no machine-readable output, no ACP/MCP/server mode, exit 0 even on total model failure, and an unasked `git init` in the working directory. The probe corrected SPEC-027 §6 (`interactive` is true, not false — Aider has an undocumented first-run OpenRouter browser sign-in that persists to `~/.aider/oauth-keys.env`) and confirmed D3 with a concrete counterexample: a host with no model env var anywhere still reached the model from that store, so env-only readiness would have called a working host not ready. |
