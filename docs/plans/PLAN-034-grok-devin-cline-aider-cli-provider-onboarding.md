...
- Phase 6 touches UI, which mirrors `cats-platform` catalog values and must follow the platform catalog landing.
...
- **Platform owns**: packaged installer helpers (`scripts/{windows,linux,macos}`), desktop host wiring (`cliInventoryProbe`, `contracts`, `setupAssets`, `packaging`, `bootstrapPage`), the shared product provider catalog, smoke tests, platform docs.

Handoff order: runtime Phase 2 (taxonomy) → platform PLAN-102 Phase 1 (shared catalog + contracts) → runtime Phase 6 (UI mirrors catalog values) → both repos' remaining phases in parallel.
...
7. Do not reorder existing providers. The four append after `kiro`.
8. Do not edit `cats-platform/src/shared/providerCatalogData.ts` from this plan — owned by platform PLAN-102 Phase 1.
...
- [ ] Grok — `platform/windows/Install-GrokCLI.ps1`, `platform/{linux,macos}/install-grok-cli.sh`. Installer `https://x.ai/cli/install.{ps1,sh}`. Binary `~/.grok/bin/grok{,.exe}` plus an `agent` alias. Auth `grok login` → `~/.grok/auth.json`, or `GROK_DEPLOYMENT_KEY`. Upstream Quick mode.
...
- [ ] Refactor `createGenericNpmKnowledge` in `src/core/provider-install/knowledge.ts` to take an explicit `binaryName`, removing the `provider === 'opencode'` special case.
...
- [ ] Add the `aider` entry per SPEC-027 §6 with `auth.interactive: false` and the model-key env var list, including the bundled-`uv` shadowing note.
- [ ] Add the `cline` entry via `createGenericNpmKnowledge`.
- [ ] Change the Pi entry's npm package to `@earendil-works/pi-coding-agent`.
- [ ] Extend `src/core/provider-install/knowledge.test.ts` to assert each new entry's binary name, expected paths, auth shape, and — for Aider — `interactive === false`.
...
- [ ] Add `src/http/providerDiagnostics.test.ts` cases covering: Grok detected via `~/.grok/bin`, an unrelated `agent` binary **not** counting as Grok, Devin present but `devin setup` still pending, and Aider present with and without model keys.
...
- [ ] Update `docs/setup-guide.md` with install commands, binary locations, and auth for all four, calling out `devin setup` and Aider's env-key model.
...
- **Append-only ordering**: keeps existing provider indices stable across two repos.
...
- **Manual**: start the runtime (`npm run dev`) and confirm —
  ...
  - Devin shows the `devin setup` manual step.
  - Aider shows env-key readiness and no sign-in button.
...
- **UI lands before the platform catalog**: Phase 6 carries an explicit blocking dependency.
- **`agent` alias false positives**: covered by a dedicated negative test in Phase 7.
- **Aider's non-interactive auth model leaks into other providers' UI**: provider-setup must branch on `auth.interactive`, not special-case the id.
