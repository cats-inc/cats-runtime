...
| 2026-03-28 | Provider-native tool inspection follow-through landed: `src/backends/api/inspection.ts` now reports payload-template-configured OpenAI and Gemini hosted tools under `apiRuntime.providerNativeTools` instead of leaving those targets indistinguishable from still-deferred provider-tool setups, and `src/backends/api/inspection.test.ts` verifies the bounded configured-tool read model. |
