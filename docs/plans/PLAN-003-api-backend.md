...
| 2026-03-28 | Continuation fallback regression follow-through landed: `tests/api-backend.test.ts` now verifies that when OpenAI rejects `previous_response_id` reuse, `cats-runtime` retries the turn with full transcript replay, surfaces the `provider_cache` fallback progress event, and still completes the turn without forcing the session through a reset/resume cycle. |
| 2026-03-28 | Cached-context fallback regression follow-through landed: `tests/api-backend.test.ts` now verifies that when Gemini rejects cached-content reuse, `cats-runtime` retries the turn with full conversation replay, surfaces the `provider_cache` fallback progress event, and clears the stale cached-content provider state instead of leaving the session stuck on an invalid cache handle. |
| 2026-03-28 | OpenAI built-in tool observability follow-through landed: `src/backends/api/transports/openai.ts` now lifts provider-native response items such as `web_search_call` into shared `progress(kind: "tool")` events with bounded metadata, and both transport-level plus route-level tests verify that these checkpoints reach the existing stream/observe contract without changing the host request shape. |
| 2026-03-28 | Provider-native tool inspection follow-through landed: `src/backends/api/inspection.ts` now reports payload-template-configured Anthropic/OpenAI/Gemini hosted tools under `apiRuntime.providerNativeTools` instead of leaving those targets indistinguishable from still-deferred provider-tool setups, and `src/backends/api/inspection.test.ts` verifies the bounded configured-tool read model. |
| 2026-03-28 | Route-level inspection coverage follow-through landed: `tests/api-backend.test.ts` now verifies that `/providers/config` surfaces `apiRuntime.providerNativeTools.state = "provider_native_configured"` plus bounded `configuredTools` values when an API target preconfigures hosted tools through `payload_template`, so hosts have a regression-locked topology read model for provider-native tool posture. |
