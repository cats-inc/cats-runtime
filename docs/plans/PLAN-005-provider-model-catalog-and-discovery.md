...
| 2026-03-27 | Manual refresh follow-through landed: `GET /providers/{provider}/models` and `GET /providers/{provider}/models/advanced` now accept additive `refresh=1|true` cache-bypass semantics so hosts can re-read dynamic catalogs after model/runtime changes without waiting for TTL expiry |
...
| 2026-03-27 | Read-model follow-through landed: bounded `modelCatalog` summaries on `/providers/config` and `/diagnostics/providers` now also expose additive `defaultModelStatus`, so hosts can tell whether the resolved default/configured model is merely configured, dynamically available, or already running without fetching the full catalog payload |
