...
- [ ] Task 1.7: Add `scripts/check-provider-releases.mjs` as the thin IO shell over the pure logic
      ...
      supports `--help`. Add `npm run watch:providers` as
...
- [ ] Task 1.13: Add `scripts/import-provider-observations.mjs` and
      ...
      deterministic TypeScript diff for human review. Add the `snapshot:providers` package script as
      `tsx scripts/import-provider-observations.mjs`; invoke it as
      `npm run snapshot:providers -- --report <path>`. Require an explicit report path and support
      `--help` and `--dry-run`. Import never moves an accepted reference. The merged TypeScript
      snapshot is the sole Phase 2 runtime input for observed upstream state; near-real-time refresh
...
- [ ] Task 1.14: Add offline fixture tests for every resolver, version scheme, channel/prerelease
      ...
      recovery, snapshot import, staleness threshold, and risk tag. Tests never call live feeds.
...
| `scripts/import-provider-observations.mjs` | Create | IO shell run under `tsx`: validate a report and render the reviewed TypeScript snapshot diff |
...
- **CI output is never a runtime input; a reviewed snapshot is the bridge.** The runtime must not
  read workflow artifacts or mutable issue text, so an explicit maintainer command imports a
  checksummed report into a compiled TypeScript snapshot that ships in `build/runtime`, and Phase 2
  reads only that. The accepted limitation, recorded deliberately: snapshot freshness is bounded by
  the runtime release cadence, so between releases the honest report is often "snapshot too old to
  know" — which is why Task 2.5 surfaces snapshot age rather than implying currency. L5
  knowledge-pack delivery later replaces this release-coupled bridge with an integrity-checked,
  rollback-capable channel that refreshes independently of a release.
...
- **Operational state lives in a pinned issue, and the deciding factor is token scope.** An orphan
  ...
  into an intention, and this repo commits straight to `main`, so branch protection is not
  available as a backstop. With the issue, the watcher runs on `contents: read` and simply lacks
  the capability.
  ...
  1.9 supplies the bootstrap and recovery path, schema validation makes tampering loud, and the
  concurrency group in Task 1.10 serializes writers. The heartbeat stays externally readable
  ...
  candidate PRs land, since that already requires write access. At that point the token-scope
  argument disappears and the orphan branch becomes the better choice.
...
| A CI artifact or mutable issue is mistaken for runtime truth | High | Phase 2 reads only the reviewed, versioned TypeScript snapshot with report provenance and checksum, and a test asserts no read path touches CI output directly |
...
| Operational state is missing, deleted, or hand-edited | Medium | Schema validation fails loudly and `--init-state` gives a one-command rebuild; the orphan-branch alternative is recorded with the condition that would make it the better choice |
...
| 2026-08-18 | Operational-state store settled on the pinned issue, decided on token scope: an orphan branch needs `contents: write`, which GitHub cannot restrict to one ref, and this repo commits straight to `main` so branch protection cannot backstop it. Recorded the condition that flips it |
