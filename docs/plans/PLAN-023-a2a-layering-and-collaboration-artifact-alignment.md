...
`cats-runtime` already has the right conceptual direction for protocol,
project-memory, and skill layering, but the actual artifact set is still mixed:
...
- `cats` and `cats-runtime` are the first-wave validation repos
...
- [ ] Confirm the exact March 2026 `project-bootstrap` artifacts and scripts
      that enter the pilot input set
- [ ] Record explicitly that those bootstrap artifacts are unverified inputs,
      not accepted production defaults
- [ ] Define first-wave validation criteria for `cats` / `cats-runtime`
      collaboration: shared reading of `AGENTS.md`, `docs/AGENT-GUIDE.md`, and
      correct read/write behavior around research, ADR, spec, and plan docs
- [ ] Freeze the pilot boundary before modifying runtime-owned docs or skills
...
- [ ] Replace the legacy `docs/a2a/agent-card.*` and `task.*` examples with a
...
- [ ] Update local docs where needed to reflect the new three-layer split:
      protocol, project memory, and skill packages
- [ ] Align `docs/AGENT-GUIDE.md`, `docs/README.md`, and `docs/terminology.md`
      with the pilot operating contract and explicit external-skill vs local
      skill distinction
- [ ] Record any material runtime-specific decisions in an ADR if the pilot
      intentionally diverges from the bootstrap reference set
...
- [ ] Add a first runtime-owned `a2a-handoff` skill package
- [ ] Add a first runtime-owned `project-memory-sync` skill package
- [ ] Keep both skills procedural and lightweight: they should reference repo
      memory docs rather than copy long-lived state into `SKILL.md`
- [ ] Extend the runtime skill-library docs/catalog metadata only as needed to
      describe these new collaboration skills truthfully
- [ ] Document how bootstrap tools such as `Initialize-Project` and
...
- [ ] Validate the same operating model in new repos created or updated through
      bootstrap tooling
- [ ] Record interpretation drift, style drift, and collaboration failures found
      in those second-wave repos
- [ ] Iterate through multiple optimization loops before declaring the model
      stable
- [ ] Open a later merge-back track only after the pilot is proven across both
      first-wave and second-wave repos
