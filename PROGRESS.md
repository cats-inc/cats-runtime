...
| Add bounded provider-config model-catalog summaries | [x] | `/providers/config` instance entries now expose a compact best-known `modelCatalog` read model (source/defaultModel/modelCount/warnings/statusCounts plus optional cached-dynamic metadata) without forcing new live discovery work during provider-topology reads |
