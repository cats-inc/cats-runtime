...
| Workspace Substrate | Completed | `audit-workspace`, `init-workspace`, and `update-workspace` now return explicit preview/apply contracts, machine-readable action plans/diff stats, approval-friendly payloads, and `*.bootstrap` review-copy behavior without owning product policy; runtime worktree maintenance diagnostics now also expose bounded current retained-session summaries for operator follow-through instead of only the last sweep result, and `/diagnostics/health` now exposes a compact retained-worktree backlog summary for polling hosts |
...
- [ ] Continue broadening provider health probes beyond the current transport-native API/local auth/model checks, provider-level tooling summaries plus bounded observability truth, agent-target `agentRuntime` inspection metadata, structured OpenClaw/Agent SDK liveProbe semantics including first-slice Agent SDK streaming validation, the first OpenClaw `tools.catalog` remote-tool slice, and additive live tool-catalog diagnostics for catalog-capable agent targets, especially broader remote tool discovery and stronger live semantic validation for later targets
- [ ] Continue hardening shared local tool runtime mutation safety beyond the current bounded `apply_patch` rollback, staged atomic `write_file` / `edit_file` replacement, symlink/junction/hardlink alias guards, preserved file modes on staged replacements, source-timestamp-preserving `copy_file`, and failed-create empty parent-directory cleanup, especially broader metadata restoration
...
| Add shared local tool runtime for API/local sessions | [x] | `list_files`, `inspect_path`, `read_file`, `read_files`, `write_file`, `create_directory`, `edit_file`, `apply_patch`, `grep`, `glob`, and `run_shell` are enforced centrally, with extended `delete_file` / `rename_file` / `copy_file` support behind the opt-in profile and source-timestamp-preserving `copy_file` follow-through in the shared runtime |
...
| Add aggregate default-target model-catalog route | [x] | `GET /providers/models` now reuses the same runtime-owned catalog service to return one model catalog per configured provider default target, so hosts can fetch a bounded multi-provider read model without fanning out per-provider requests |
...
- [ ] Add broader remote tool discovery beyond the current OpenClaw `tools.catalog` plus Agent SDK bridge provider-registry slices and stronger later-target semantic probes beyond the current OpenClaw gateway health/model checks and Agent SDK bridge provider-registry/model/tool/streaming visibility coverage
...
- [ ] No full transactional rollback for shared local-tool writes/edits yet; `apply_patch` now restores bounded file content/presence on failure, while `write_file` / `edit_file` preserve existing file modes and clean empty parent dirs after failed new-file writes, and `copy_file` now preserves source timestamps, but broader metadata restoration still remains
...
| Expose curated read + mutation tool slice | [x] | `runtime_summary`, `runtime_diagnostics`, `list_sessions`, `read_session`, `session_history`, `session_lineage`, `health_diagnostics`, `pool_status`, `management_diagnostics`, `providers_config`, `provider_tools`, `provider_models`, `providers_models`, `provider_advanced_models`, `provider_diagnostics`, `reprobe_provider_diagnostics`, `list_compatibility_evidence_artifacts`, `read_compatibility_evidence_artifact`, `list_provider_evolution_artifacts`, `read_provider_evolution_artifact`, `review_provider_evolution_artifact`, `generate_setup_diagnostic_report`, `list_setup_diagnostic_reports`, `read_latest_setup_diagnostic_report`, `read_setup_diagnostic_report`, `setup_state`, `run_setup_scan`, `apply_setup_config`, `observe_session`, `list_wakeups`, `read_wakeup`, `create_wakeup`, `cancel_wakeup`, `trigger_wakeup`, `list_runtime_skills`, `list_browser_drivers`, `list_browser_sessions`, `browser_summary`, `create_browser_session`, `create_browser_page`, `navigate_browser_page`, `close_browser_page`, `close_browser_session`, `cleanup_browser_sessions`, `create_session`, `send_message`, `close_session`, `reset_session`, `fork_session`, `delete_session`, `cleanup_session_workspace`, `compact_session`, `report_session_maintenance_follow_through`, `report_compaction_follow_through`, `audit_workspace`, `init_workspace`, `audit_delivery_target`, and `commit_changes` now ship; runtime/health MCP tools now mirror the same aggregate diagnostics surfaces as `/diagnostics/runtime` and `/diagnostics/health`, `pool_status` and `management_diagnostics` now mirror the same operator read surfaces as `/pool/status` and `/management/diagnostics`, `read_session` now mirrors the same session detail surface as `GET /sessions/{id}`, `session_history` and `session_lineage` now mirror the same transcript and branch-lineage surfaces as `GET /sessions/{id}/history` and `GET /sessions/{id}/lineage`, `list_wakeups` and `read_wakeup` now mirror the same bounded wakeup inspection surfaces as `GET /wakeups` and `GET /wakeups/{id}`, `create_wakeup`, `cancel_wakeup`, and `trigger_wakeup` now mirror the same bounded wakeup lifecycle seams as `POST /wakeups`, `POST /wakeups/{id}/cancel`, and `POST /wakeups/{id}/trigger`, `providers_config` now mirrors the same topology/inspection surface as `/providers/config`, `provider_tools` now mirrors the same bounded tooling inspection surface as `/providers/{provider}/tools`, `provider_models` now mirrors the same per-target model catalog surface as `/providers/{provider}/models`, `providers_models` now mirrors the same aggregate configured-provider catalog surface as `/providers/models`, `provider_advanced_models` now mirrors the same additive advanced per-target model catalog surface as `/providers/{provider}/models/advanced`, `provider_diagnostics` now mirrors the same target filters as `/diagnostics/providers`, explicit reprobe now mirrors `POST /diagnostics/providers/reprobe`, retained compatibility/provider-evolution artifact tools now mirror the same bounded diagnostics list/read/review filters as `/diagnostics/providers/evidence` and `/diagnostics/providers/evolution`, setup diagnostic MCP tools now mirror the same generate/list/latest/read lifecycle as `/diagnostics/setup-report`, setup-state MCP reads now mirror the same shared repair/next-action model as `/setup-state`, setup workflow MCP mutations now mirror the same bounded manual-scan/apply-config seams as `/setup-scan` and `/setup-apply`, and MCP `list_sessions` / `observe_session` now reuse the same additive session `providerTarget` continuity/tooling summary surfaced by the direct HTTP session reads |
