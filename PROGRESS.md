...
| Browser Preview Substrate | Completed | Runtime-owned browser driver/session/page contracts, `browser_page` preview surfaces, a persistent manual driver plus an opt-in Playwright driver, driver-aware restart recovery for non-persistent browser sessions, additive `/browser/*` routes including single-page close, session/history/observe inspection integration, aggregate browser summary/cleanup seams, background closed-session maintenance, truthful downgraded preview state for closed pages, and reset/delete cleanup for runtime-bound browser sessions now exist without depending on sibling browser projects |
...
| Add browser HTTP routes | [x] | `/browser/drivers`, `/browser/summary`, `/browser/sessions`, `/browser/sessions/{id}`, `/browser/sessions/cleanup`, `/browser/sessions/{id}/pages`, `/browser/sessions/{id}/pages/{pageId}/close`, and `/browser/sessions/{id}/close` now ship |
...
- [ ] No richer page-level lifecycle yet beyond open/close semantics and preview-state downgrade for closed pages
