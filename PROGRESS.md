...
- [ ] Add stronger later-target semantic probes beyond the current OpenClaw gateway health/model/tool checks plus session-scoped `tools.effective` coverage on tooling/diagnostics read surfaces and beyond the current Agent SDK bridge provider-registry/model/tool-metadata/streaming visibility plus bounded probe-session create/read/delete validation, especially dedicated non-OpenClaw remote-tool/service evidence beyond provider-registry truth
