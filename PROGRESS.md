...
| Peer Discovery & Routing | Completed | PLAN-017 v0 now includes bounded peer registry/discovery diagnostics, dedicated `POST /peer/executions`, additive `routing` on `POST /sessions/{id}/messages`, trust-gated runtime-to-runtime execution, and caller-owned observe/stream relay semantics without changing existing `cats` contracts |
...
| Add persisted maintenance follow-through envelope | [x] | `POST /sessions/{id}/maintenance/follow-through` plus matching MCP tooling now let hosts acknowledge, retry, and report completion for `pre_reset`, `pre_compaction`, and `pre_flush`; reset/delete/workspace-cleanup can also opt into `requireAcknowledgedHooks`, and `POST /sessions/{id}/compact/follow-through` remains as the compaction shortcut |
