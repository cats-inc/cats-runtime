...
- [ ] Add stronger later-target semantic probes beyond the current OpenClaw gateway health/model/tool checks plus session-scoped `tools.effective` coverage on tooling/diagnostics read surfaces and beyond the current Agent SDK bridge provider-registry/model/tool/streaming visibility plus bounded probe-session lifecycle validation
