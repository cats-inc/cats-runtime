...
| Land the first manual `PLAN-021` probe entrypoint with snapshot/baseline compare | [x] | `cats-runtime --probe-provider-evolution --probe-provider <provider>` now runs a bounded manual probe for the first six high-value CLI adapters plus Junie and Auggie as bounded JSON-output targets and Kilo/OpenCode as native-session-service targets, persists a machine-readable artifact under `<dataDir>/compatibility/provider-evolution/`, derives capability snapshots, and compares against the latest matching baseline artifact |
