...
| Peer Discovery & Routing | Completed | PLAN-017 v0 now includes bounded peer registry/discovery diagnostics, dedicated `POST /peer/executions`, additive `routing` on `POST /sessions/{id}/messages`, trust-gated runtime-to-runtime execution, bounded auth-failure throttling plus inbound admission control on peer-only routes, and caller-owned observe/stream relay semantics without changing existing `cats` contracts |
