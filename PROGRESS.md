...
| Docs | In Progress | Core docs now cover startup/diagnostics, provider compatibility/evidence flows, model catalog, session branching, worktree-backed session isolation and cleanup, workspace substrate, runtime hydration/re-entry metadata, delivery primitives, the browser preview substrate including opt-in Playwright driver enablement plus non-persistent restart recovery, single-page close semantics, and explicit idle-ready cleanup support, runtime-managed skills plus the internal skill-library taxonomy/metadata contract, the scheduled wakeup substrate, first-slice metering/progress contracts, additive session inspection/run-state/maintenance payloads including sanitized persisted maintenance requests plus generic maintenance follow-through outcomes, bounded maintenance request/follow-through history, additive `maintenance.flush` read-model state, the runtime MCP facade with authoritative HTTP ownership on `POST /mcp` plus the published `cats-runtime mcp` stdio proxy entrypoint and repo-local `node build/runtime/bin/mcp.js` equivalent, the ACP provider/runtime-facade stack including `SPEC-025`, `PLAN-032`, bounded `/acp` surface docs, clarified ACP-versus-A2A layering terminology/architecture notes, explicit runtime/health ACP coexistence diagnostics, ACP proxy preflight guidance for `cats-runtime acp --inspect-proxy`, dedicated nested ACP `launch:`, `connect:`, and `request:` config guidance in `SPEC-025` plus setup/deployment docs, additive `read_only` runtime-tool profile summaries on runtime diagnostics, explicit WSL manual-discovery `startIfNeeded` override truth on provider-native discovery routes, compact runtime/health discovery summaries alongside `GET /discovery/status`, PLAN-017 LAN peer discovery/execution routing v0 including peer network-posture diagnostics, the broader `SPEC-006` pilot including the repo-owned A2A v1 example set, same-environment collaboration skills, repo-owned workspace starter/update helpers, split-safe starter validation, sibling alignment notes against `cats-platform`, the current `SPEC-017` / `PLAN-019` truth that shared shell/manual-repair/HTML-emit slices are landed while page-entry modularization remains open, and the completed `PLAN-025` repo-owned packaging prep including frozen unscoped `cats-runtime` naming plus a non-publishing GitHub Actions release preflight; broader browser-driver follow-through and remaining PLAN-003 or PLAN-005 follow-on items still need ongoing updates |
...
| Land the first `PLAN-021` provider-evolution collector/instrumentation slice | [x] | `src/core/compatibility/providerEvolution.ts` now owns a transport-neutral evidence bundle/observer shape, and Codex/Copilot/Cursor/Pi/Goose/Gemini/Claude parsers can emit `normalized`, `ignored`, `unknown`, `schema_failure`, and `raw_passthrough` observations without changing normal runtime behavior when collection is disabled |
| Land the first manual `PLAN-021` probe entrypoint with snapshot/baseline compare | [x] | `cats-runtime --probe-provider-evolution --probe-provider <provider>` now runs a bounded manual probe for the first six high-value CLI adapters plus Cursor as the next stream-json target, Junie and Auggie as bounded JSON-output targets, and Kilo/OpenCode as native-session-service targets, persists a machine-readable artifact under `<dataDir>/compatibility/provider-evolution/`, derives capability snapshots, and compares against the latest matching baseline artifact |
...
| Add standalone stdio MCP transport | [x] | The published `cats-runtime mcp` entrypoint plus the repo-local `node build/runtime/bin/mcp.js` equivalent now expose the same tool plane over Content-Length framed stdio JSON-RPC, proxy normal MCP traffic to the primary runtime's authoritative `POST /mcp` surface, and ignore generic `PORT` so hosted shell environments do not accidentally retarget the proxy away from the runtime |
