...
| Bootstrap / Setup | In Progress | Standalone bootstrap mode, generated `providers.yaml`, persisted setup-state/manual-scan artifacts, provider-first setup routes, bounded concurrent provider scan orchestration, a shared `GET /setup-state` read model with repair/next-action metadata plus actionable `repair.actions`, ready-to-apply provider lists, remediation previews, and the latest setup-report snapshot including a concise report headline plus bounded highlights, and a redacted setup diagnostic report service across persisted `/diagnostics/setup-report` artifacts including newest-first retained report summaries plus read-by-id retrieval, additive operator-facing `summary.headline` / `summary.highlights` fields plus bounded `references.providerEvolutionArtifacts[]` metadata for the latest retained provider-evolution evidence, and non-server CLI entry paths for `--diagnose-setup`, `--list-setup-diagnostic-reports`, and `--read-setup-diagnostic-report` that keep stdout JSON machine-readable while emitting concise stderr summaries are in-repo; shared UI follow-through and broader repair flows remain |
