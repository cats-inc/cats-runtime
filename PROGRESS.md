...
| Add persisted provider-evolution artifact review and internal read models | [x] | Probe artifacts now persist bounded review classifications/highlights alongside compare output, and `ProviderEvolutionProbeService` can list the latest/recent/read-by-id summaries without introducing a public HTTP surface yet |
...
| Extend `PLAN-021` manual probes to agent-backed targets | [x] | The shared probe entrypoint now resolves `agent/<instance>` targets, persists agent-transport artifacts alongside CLI baselines, and the first Agent SDK bridge instrumentation records ignored/unknown/raw-passthrough/schema-failure paths plus normalized `tool_result` output without changing normal session execution |
...
- [ ] No public HTTP/diagnostics surface yet for `PLAN-021` probe artifacts; the flow remains CLI/manual-first
