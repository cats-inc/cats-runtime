...
- [ ] Add stronger later-target semantic probes beyond the current OpenClaw gateway health/model/tool checks plus session-scoped `tools.effective` coverage and beyond the current Agent SDK bridge provider-registry/model/tool/streaming visibility coverage
