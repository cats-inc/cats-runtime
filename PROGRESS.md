...
| Management Adapters | Completed | Runtime-owned management adapters for non-session control-plane tools (GitHub CLI review, Zeabur CLI deployment) with 8 actions across 2 domains, product-neutral authorization, bounded long-poll for review checks, `RuntimePreviewSurface` reuse for deployment URLs, dedicated HTTP/MCP/local-tool surfaces, adapter diagnostics, additive retained-operation summary/recent read models, and `config/management.yaml` configuration |
...
- [ ] Continue hardening shared local tool runtime mutation safety beyond the current bounded `apply_patch` rollback, staged atomic `write_file` / `edit_file` replacement, symlink/junction/hardlink alias guards, preserved file modes plus overwrite-time `atime` / `mtime` restoration on staged replacements, source-timestamp-preserving `copy_file`, and failed-create empty parent-directory cleanup, especially broader metadata restoration beyond those basics
- [ ] Continue broadening the shared local tool runtime beyond the current filesystem/shell/navigation/materialization set plus proposed-file diff, bounded batch-read inspection, and bounded batch path-inspection planning helpers, especially richer planning helpers beyond those read-oriented seams
- [ ] Continue refining capability partitioning and policy surfacing beyond the current profile summary/read-model slice, additive runtime-local profile-catalog truth on provider tooling read surfaces, per-tool capability/access metadata, and `standard` / `extended` / `read_only` split
...
| Add shared local tool runtime for API/local sessions | [x] | `list_files`, `inspect_path`, bounded batch `inspect_paths`, `read_file`, `read_files`, `diff_file`, `write_file`, `create_directory`, `edit_file`, `apply_patch`, `grep`, `glob`, and `run_shell` are enforced centrally, with extended `delete_file` / `rename_file` / `copy_file` support behind the opt-in profile, overwrite-time `atime` / `mtime` restoration on staged `write_file` / `edit_file` replacements, source-timestamp-preserving `copy_file`, and additive runtime-local tool profile catalog truth on provider tooling read surfaces |
...
- [ ] No full transactional rollback for shared local-tool writes/edits yet; `apply_patch` now restores bounded file content/presence on failure, while `write_file` / `edit_file` preserve existing file modes plus overwrite-time `atime` / `mtime`, clean empty parent dirs after failed new-file writes, and `copy_file` preserves source timestamps, but broader metadata restoration still remains
...
| Update MCP docs and coverage | [x] | `docs/api.md`, `docs/architecture.md`, `docs/mcp-config.md`, `../../cats/docs/mcp-config.md`, `src/http/mcpRoutes.test.ts`, `src/mcp/proxy.test.ts`, and `src/mcp/stdio.test.ts` now describe and verify the slice |
