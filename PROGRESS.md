...
- [ ] Continue refining capability partitioning and policy surfacing beyond the current runtime-local profile summary/read-model slice, per-tool capability/access catalog truth on provider tooling read surfaces, and `standard` / `extended` / `read_only` split, especially finer-grained session/workspace-policy overlays beyond those baseline partitions
