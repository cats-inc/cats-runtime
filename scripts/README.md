...
| Script | Platform | Purpose |
|--------|----------|---------|
| `scripts/sync-agent-skills.mjs` | Cross-platform | Reconcile canonical `skills/` into ignored agent discovery mirrors |
...
repository-managed entries. Runtime-delivered packages under `runtime-skills/` are not synced.
...
The runtime's `WorkspaceSubstrateService` and this repository's helpers both use `skills/`
for developer workflows. The shipped product library uses `runtime-skills/`; generated
workspace helpers never synchronize that product library (ADR-036).
